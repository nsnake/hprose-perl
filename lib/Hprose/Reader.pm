...
my $read_class =
  sub { my $self = shift; $self->read_class; $self->read_object; };
...
my $read_list_without_tag = sub { shift->read_list_without_tag };
...
sub new {
    ...
    if ($simple) {
        ...
        $refer->{read} = sub {
            my $self = shift;
            $self->{refer}->{ref}
              ->[ readint( $self->{stream}, Hprose::Tags->Semicolon ) ];
        };
...
sub check_tags {
    ...
    $tag = $self->{stream}->getc if ( !defined($tag) );
    $self->unexpected_tag( $tag, $expect_tags )
...
sub unserialize {
    ...
    ( defined($tag) && exists( $unserializeMethod{$tag} ) )
...
sub read_integer {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readIntegerMethod{$tag} ) )
...
sub read_long {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readLongMethod{$tag} ) )
...
sub read_double_without_tag {
    Math::BigFloat->new(
        readuntil( shift->{stream}, Hprose::Tags->Semicolon ) );
}
...
sub read_double {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readDoubleMethod{$tag} ) )
...
sub read_infinity {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( $tag eq Hprose::Tags->Infinity )
      ? $self->read_infinity_without_tag
      : $self->unexpected_tag($tag);
}
...
sub read_boolean {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readBooleanMethod{$tag} ) )
...
sub read_date_without_tag {
    ...
    my ( $year, $month, $day, $hour, $minute, $second, $nanosecond ) =
      ( 1970, 1, 1, 0, 0, 0, 0 );
...
sub read_date {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readDateMethod{$tag} ) )
...
sub read_time {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readTimeMethod{$tag} ) )
...
sub read_bytes {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readBytesMethod{$tag} ) )
...
sub read_utf8char {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( $tag eq Hprose::Tags->UTF8Char )
      ? $self->read_utf8char_without_tag
      : $self->unexpected_tag($tag);
}
...
my %readStringMethod = (
    ...
    Hprose::Tags->UTF8Char => \&read_utf8char_without_tag,
    Hprose::Tags->String   => $read_string_without_tag,
    Hprose::Tags->Null     => sub { return undef }
);

sub read_string {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readStringMethod{$tag} ) )
...
sub read_guid {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readGuidMethod{$tag} ) )
...
sub read_array {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readArrayMethod{$tag} ) )
...
sub read_hash {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readHashMethod{$tag} ) )
...
sub read_object_without_tag {
    ...
    my $stream   = $self->{stream};
    my $classref = $self->{classref};
    my ( $class, $fields, $count ) =
      @{ $classref->[ readint( $stream, Hprose::Tags->Openbrace ) ] };
    my $object = $class->new;
...
sub read_object {
    my $self = shift;
    my $tag  = $self->{stream}->getc;
    ( defined($tag) && exists( $readObjectMethod{$tag} ) )
...
sub read_list_without_tag {
    my $self = shift;
    my $list = [];
    $self->{refer}->{set}( $self, $list );
    my $stream = $self->{stream};
    my $count = readuntil( $stream, Hprose::Tags->Openbrace );
    $list->[$_] = $self->unserialize foreach ( 0 .. $count - 1 );
    $stream->getc;
    return $list;
}
